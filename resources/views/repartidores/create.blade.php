...
@section('content')
<div class="mb-6">
    <h1 class="text-2xl font-semibold">Nuevo Repartidor</h1>
</div>
...
<div class="bg-white rounded-lg shadow p-6 max-w-lg">
    <form action="{{ route('repartidores.store') }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="email" name="email" id="email" value="{{ old('email') }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 text-sm" required>
            @error('email') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
        </div>

        <div class="mb-4">
            <label for="phone" class="block text-sm font-medium text-gray-700 mb-1">Teléfono</label>
            <input type="text" name="phone" id="phone" value="{{ old('phone') }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 text-sm">
        ...
        <div class="grid grid-cols-2 gap-4 mb-4">
            <div>
                ...
                <select name="vehicle_type" id="vehicle_type" class="w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 text-sm">
                    ...
                    <option value="coche" {{ old('vehicle_type') === 'coche' ? 'selected' : '' }}>Coche</option>
                    <option value="bici" {{ old('vehicle_type') === 'bici' ? 'selected' : '' }}>Bicicleta</option>
                    <option value="pie" {{ old('vehicle_type') === 'pie' ? 'selected' : '' }}>A pie</option>
                </select>
            ...
            <div>
                ...
                <input type="text" name="license_plate" id="license_plate" value="{{ old('license_plate') }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 text-sm">
        ...
        <div class="mb-4">
            ...
            <select name="status" id="status" class="w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 text-sm">
                ...
                <option value="busy" {{ old('status') === 'busy' ? 'selected' : '' }}>Ocupado</option>
